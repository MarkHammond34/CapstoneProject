...
public class ListingDAOImpl implements ListingDAO {
	...
	public List<Listing> getActiveListingsUserBidOn(int userID) {
		Query q = getSession().createSQLQuery(
				"SELECT * FROM listing AS l WHERE l.active=1 AND l.id IN (SELECT lb.listing_id FROM listing_bid AS lb WHERE userID=:userID)")
				.addEntity(Listing.class);
	...
	public List<Listing> getListingsLost(int userID) {
		Query q = getSession().createSQLQuery(
				"SELECT * FROM listing AS l WHERE l.active=0 AND l.highest_bid_userID!=:userID AND l.id IN (SELECT lb.listing_id FROM listing_bid AS lb WHERE userID=:userID)")
				.addEntity(Listing.class);
		q.setParameter("userID", userID);
		return (List<Listing>) q.list();
	}
	...
	public List<Listing> getListingsWon(int userID) {
		Query q = getSession().createSQLQuery("SELECT * FROM listing WHERE active=0 AND highest_bid_userID=:userID")
				.addEntity(Listing.class);
		q.setParameter("userID", userID);
		return (List<Listing>) q.list();
	}
